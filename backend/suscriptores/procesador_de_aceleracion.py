...
#           |     Procesador de     |    aceleraciones de     |    Xiaomi My Band.     |
#           |     aleraciones       |    un cuerpo.           |  - Define las metricas |
#           |                       |                         |    para detectar una   |
...
class ProcesadorAceleracion:
    def consume(self):
        try:
            ...
            channel.basic_consume(queue='accelerations', on_message_callback=self.callback, auto_ack=False)
    ...
    def callback(self, ch, method, properties, body):
        ...
        if (float(json_message['valueX']) > .6 and float(json_message['valueY']) < .5 and float(json_message['valueZ']) > -.5):
            monitor = Monitor()
            monitor.print_notification_trip(json_message['date'], json_message['id'], json_message['model'])
        time.sleep(1)
