...
#           |     Procesador de     |    en que le toca la    |    Xiaomi My Band.     |
#           |        medicinas      |    medicina a un        |                        |
#           |                       |    adulto.              |                        |
#           |                       |                         |                        |
#           |                       |                         |  - Notifica al monitor |
#           |                       |                         |    cuando le toca la   |
#           |                       |                         |    medicina a un adulto|
#           +-----------------------+-------------------------+------------------------+
#
...
#           |                        |  - method: propio de     |    la medicina y hora |
#           |                        |     Rabbit.              |    de un adulto.      |
...
class ProcesadorMedicinas:
    ...
    def consume(self):
        try:
            ...
            channel.basic_consume(queue='medicine', on_message_callback=self.callback, auto_ack=False)
    ...
    def callback(self, ch, method, properties, body):
        ...
        self.adultos.append(json_message)
        tiempo = time.strftime("%H %M")
        adultos_2 = []
        for adulto in self.adultos:
            if ((adulto['time'] == tiempo)):
                monitor = Monitor()
                monitor.print_notification_medicine(adulto['time'], adulto['value'], adulto[
                                            'dose'],json_message['id'], json_message['model'])
            else:
                adultos_2.append(adulto)

        self.adultos = adultos_2
        time.sleep(1)
...
if __name__ == '__main__':
    p_medicinas = ProcesadorMedicinas()
    p_medicinas.consume()
